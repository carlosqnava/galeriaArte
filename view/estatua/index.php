            ...
            <div class="tile-body">
              <table class="table table-hover table-bordered" id="sampleTable">
                <thead>
                  ...
                  <tbody>
                   <?php foreach($this->modelo->getEstatua() as $Est): ?>
                    <tr>
                        <th scope="row"><?= $Est->idobj?></th>
                        <td><?= $Est->materialEst?></td>
                        <td>
                          <a href="?c=objetoArte&a=FormAgregar&id=<?=$Est->idobj?>" class="btn btn-info btn-flat">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Eliminar estatua..</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <p>Estas seguro de eliminar la estatua?</p>
...
<script type="text/javascript">
  $(function(){
    ...
    $('#btneliminar').on('click', function(event){
      ...
      var url = "?c=estatua&a=Eliminar&id="+ idobj;
